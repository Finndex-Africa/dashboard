...
function parseAdminDashboardStats(
  response: unknown,
  ...
  const payload =
    ...
    "properties" in (root.data as object)
      ? (root.data as AdminDashboardStats)
      : "properties" in root
        ? (root as unknown as AdminDashboardStats)
        : null;
  if (!payload?.properties || !payload?.services || !payload?.users)
    return null;
  return payload;
}
...
export default function AdminDashboard() {
      ...
      try {
        ...
        const [pRes, sRes, uRes, nRes, statsRes] = await Promise.all([
          propertiesApi
            ...
            .catch(() => ({ data: [] })),
          servicesApi
            .getAllAdminServices({ page: 1, limit: 10 })
            .catch(() => ({ data: [] })),
  ...
  const propTotal =
    adminStats?.properties?.total ??
    extractTotal(rawResponses?.pRes, properties.length);
  const svcTotal =
    adminStats?.services?.total ??
    extractTotal(rawResponses?.sRes, services.length);
  ...
  const monthlyData = (() => {
    const filter = (arr: any[], mi: number) =>
      arr.filter((x) => {
    ...
    if (selectedMonth !== "all") {
      ...
      return [
        {
          month: months[mi],
          properties: filter(properties, mi),
          services: filter(services, mi),
          users: filter(users, mi),
        },
      ];
    }
    return months.map((m, i) => ({
      month: m,
      properties: filter(properties, i),
      services: filter(services, i),
      users: filter(users, i),
    }));
  })();
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={16}>
            <Card style={{ borderRadius: 16, height: 420 }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, #0000FF 0%, #0033CC 50%, #0055EE 100%)",
          borderRadius: 20,
              ...
              {[
                ...
                >
                  ...
                  <div>
                    <Text
                      type="secondary"
                      style={{ fontSize: 12, display: "block", lineHeight: 1 }}
                    >
                      {item.label}
                    </Text>
                  ...
                  {typeData.map((item, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        marginBottom: 4,
                        background:
                          idx === 0 ? `${pieColors[0]}08` : "transparent",
                      }}
                    >
                      <div
                        style={{
                      ...
                      >
                        ...
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          (
                          {propTotal > 0
                            ? Math.round((item.value / propTotal) * 100)
                            : 0}
                          %)
                        </Text>
                      </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {topProps.length > 0 ? (
                topProps.map((prop, idx) => {
                  ...
                  const pct =
                    maxPerf > 0
                      ? Math.round(((prop.perf || 0) / maxPerf) * 100)
                      : 0;
